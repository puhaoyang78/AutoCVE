...
from ..core.message import MessagePriority, MessageType, message_bus
...
class CreateAgentInput(BaseModel):
    ...
    knowledge_modules: str | None = Field(
        default=None,
...
注意：每个Agent最多加载5个知识模块。"""
    ...
    async def _execute(
        ...
        knowledge_modules: str | None = None,
...
class AgentFinishInput(BaseModel):
    ...
    findings: list[str] | None = Field(default=None, description="发现列表")
    success: bool = Field(default=True, description="是否成功")
    recommendations: list[str] | None = Field(default=None, description="建议列表")

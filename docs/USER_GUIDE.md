...
本文档将帮助用户快速了解并使用 AutoCVE，内容涵盖环境部署、模型配置、项目导入、Agent 审计、一键挖掘 CVE、漏洞管理和 Skills 管理等核心功能。
...
- 数据库：PostgreSQL 15
- 缓存/任务状态：Redis 7
- 沙箱镜像：`autocve-sandbox:latest`，用于安全工具执行和 PoC 验证
...
AutoCVE 支持为不同 Agent 单独配置模型。当前可配置的 Agent 包括：
